...
@testset "MemorylessNonlinearities.jl" begin
    ...
    @test filt(SαSNL(2.0, 1.0, 0.0, false), x) ≈ x ./ 2 atol=0.0001
    @test filt(SαSNL(1.0, 1.0, 0.0, false), x) ≈ [-0.6,-0.8,-1.0,0.0,1.0,0.8,0.6] atol=0.0001
    @test filt(SαSNL(2.0, 1.0, 0.0, true), x) ≈ x ./ 2 atol=0.0001
    @test filt(SαSNL(1.0, 1.0, 0.0, true), x) ≈ [-0.6,-0.8,-1.0,0.0,1.0,0.8,0.6] atol=0.0001
    @test filt(SαSNL(2.0, 1.0, 0.0), x) ≈ x ./ 2 atol=0.0001
    @test filt(SαSNL(1.0, 1.0, 0.0), x) ≈ [-0.6,-0.8,-1.0,0.0,1.0,0.8,0.6] atol=0.0001
    @test filt(SαSNL(2.0), x) ≈ x ./ 2 atol=0.0001
    @test filt(SαSNL(1.0), x) ≈ [-0.6,-0.8,-1.0,0.0,1.0,0.8,0.6] atol=0.0001
    ...
    for α in αs
        @test filt(SαSNL(α, 1.0, 0.0, true), xtmp) ≈ filt(SαSNL(α, 1.0, 0.0, false), xtmp) rtol=0.01
        @test filt(SαSNL(α, 2.0, 0.0), xtmp) ≈ filt(SαSNL(α, 2.0, 0.0, false), xtmp) rtol=0.01
        @test filt(SαSNL(α), xtmp) ≈ filt(SαSNL(α, 1.0, 0.0, false), xtmp) rtol=0.01
    end
